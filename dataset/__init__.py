...
def create_dataset(dataset, config):
    ...
    visual_tokenizer_transform = transforms.Compose([                        
            transforms.RandomResizedCrop(config['image_res'] // 2, scale=(0.2, 1.0), interpolation=Image.BICUBIC),
            transforms.RandomHorizontalFlip(),
            RandomAugment(2,7,isPIL=True,augs=['Identity','AutoContrast','Equalize','Brightness','Sharpness',
                                              'ShearX', 'ShearY', 'TranslateX', 'TranslateY', 'Rotate']),     
            transforms.ToTensor(),
    ...
    if dataset=='pretrain':
        dataset = pretrain_dataset(config['train_file'], pretrain_transform, visual_tokenizer_transform, image_resolution=config['image_res'])                  
        return dataset      
