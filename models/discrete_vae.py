...
class DiscreteVAE(BasicVAE):
    def __init__(
        ...
        for layer_id in range(num_layers):
            enc_layers.append(
                nn.Sequential(
                    nn.Conv2d(enc_in, hidden_dim, 4, stride=2, padding=1), nn.ReLU()
                )
            )
            enc_layers.append(
                ResBlock(
            ...
            dec_layers.append(
                nn.Sequential(
                    nn.ConvTranspose2d(dec_in, hidden_dim, 4, stride=2, padding=1),
                    nn.ReLU(),
                )
            )
            dec_layers.append(
...
class Dalle_VAE(BasicVAE):
    ...
    def decode(self, img_seq):
        ...
        z = (
            F.one_hot(img_seq, num_classes=self.encoder.vocab_size)
            .permute(0, 3, 1, 2)
            .float()
        )
    ...
    def forward(self, img_seq_prob, no_process=False):
        if no_process:
            ...
            z = img_seq_prob.view(
                bsz, self.image_size // 8, self.image_size // 8, self.encoder.vocab_size
            )
