...
class SIMLA(nn.Module):
    ...
    def forward(self, image, text, visual_token_ids, masked_visual_token_pos, masked_visual_tok_labels, alpha=0):
        ...
        with torch.no_grad():
            ...
            concept_hard_targets = torch.zeros(batch_size, self.tokenizer.vocab_size, device=image.device)
        ...
        loss_pseudolabels_hard = F.binary_cross_entropy_with_logits(
            pseudolabel_predictions,
            concept_hard_targets
        )
        loss_pseudolabels_soft = F.binary_cross_entropy_with_logits(
            pseudolabel_predictions,
            concept_soft_targets
        )
        loss_pseudolabels = (1 - alpha) * loss_pseudolabels_hard + alpha * loss_pseudolabels_soft
